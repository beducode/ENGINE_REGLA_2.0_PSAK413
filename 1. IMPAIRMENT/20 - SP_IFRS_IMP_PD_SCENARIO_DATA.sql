...
BEGIN 
    ...
    IF P_PRC = 'S' THEN
        ...
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'SELECT DUPLICATE_TABLE(LOWER(''IFRS_PD_RULES_CONFIG''),LOWER(''' || V_TABLEPDCONFIG || '''), TRUE)';
        EXECUTE (V_STR_QUERY);
        ...
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'SELECT DUPLICATE_TABLE(LOWER(''TMP_IFRS_PD_SCENARIO_DATA''),LOWER(''' || V_TABLEINSERT4 || '''), FALSE)';
        EXECUTE (V_STR_QUERY);
        ...
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'SELECT DUPLICATE_TABLE(LOWER(''IFRS_SCENARIO_SEGMENT_GENERATE_QUERY''),LOWER(''' || V_TABLESEGMENT || '''), TRUE)';
        EXECUTE (V_STR_QUERY);
    END IF;
    -------- ====== PRE SIMULATION TABLE ======
    ...
    FROM ' || V_TABLEPDCONFIG || ' A INNER JOIN  ' || V_TABLESEGMENT || ' B ON A.SEGMENTATION_ID= B.RULE_ID   AND B.SEGMENT_TYPE = ''PD_SEGMENT''    
