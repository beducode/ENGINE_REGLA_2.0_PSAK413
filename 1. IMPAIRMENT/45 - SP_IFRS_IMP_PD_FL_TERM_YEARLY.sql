...
BEGIN 
    ...
    IF P_PRC = 'S' THEN 
        ...
        V_TABLEINSERT5 := 'IFRS_PD_TERM_STRUCTURE_NOFL_YEARLY_' || P_RUNID || '';
        V_TABLEINSERT6 := 'IFRS_ECL_MODEL_DETAIL_PD_' || P_RUNID || '';
        V_TABLEINSERT7 := 'IFRS_ECL_MODEL_HEADER_' || P_RUNID || '';
        V_TABLEINSERT8 := 'IFRS_ME_FL_SCALAR_RESULT_' || P_RUNID || '';
        V_TABLEINSERT9 := 'VW_IFRS_MAX_BUCKET_' || P_RUNID || '';
    ...
    ELSE 
        ...
        V_TABLEINSERT5 := 'IFRS_PD_TERM_STRUCTURE_NOFL_YEARLY';
        V_TABLEINSERT6 := 'IFRS_ECL_MODEL_DETAIL_PD';
        V_TABLEINSERT7 := 'IFRS_ECL_MODEL_HEADER';
        V_TABLEINSERT8 := 'IFRS_ME_FL_SCALAR_RESULT';
        V_TABLEINSERT9 := 'VW_IFRS_MAX_BUCKET';
    ...
    IF P_PRC = 'S' THEN
        ...
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'CREATE TABLE ' || V_TABLEINSERT4 || ' AS SELECT * FROM IFRS_IMP_PD_FL_TERM_YEARLY';
        EXECUTE (V_STR_QUERY);
        ...
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'CREATE TABLE ' || V_TABLEINSERT5 || ' AS SELECT * FROM IFRS_PD_TERM_STRUCTURE_NOFL_YEARLY';
        EXECUTE (V_STR_QUERY);
        ...
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'CREATE TABLE ' || V_TABLEINSERT6 || ' AS SELECT * FROM IFRS_ECL_MODEL_DETAIL_PD';
        EXECUTE (V_STR_QUERY);
        ...
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'CREATE TABLE ' || V_TABLEINSERT7 || ' AS SELECT * FROM IFRS_ECL_MODEL_HEADER';
        EXECUTE (V_STR_QUERY);
        ...
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'CREATE TABLE ' || V_TABLEINSERT8 || ' AS SELECT * FROM IFRS_ME_FL_SCALAR_RESULT';
        EXECUTE (V_STR_QUERY);
        ...
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'CREATE TABLE ' || V_TABLEINSERT9 || ' AS SELECT * FROM VW_IFRS_MAX_BUCKET';
        EXECUTE (V_STR_QUERY);
    END IF;
    -------- ====== PRE SIMULATION TABLE ======
    ...
    INNER JOIN ' || V_TABLEINSERT6 || ' B ON A.PKID = B.EIL_MODEL_ID 
    INNER JOIN ' || V_TABLEINSERT5 || ' C ON B.PD_MODEL_ID = C.PD_RULE_ID AND B.EFF_DATE = C.DOWNLOAD_DATE
    LEFT JOIN ' || V_TABLEINSERT8 || ' D ON B.ME_MODEL_ID = D.MODEL_ID AND B.SCALAR_EFF_DATE = D.PERIOD_DATE AND C.FL_YEAR = D.FL_YEAR
    LEFT JOIN ' || V_TABLEINSERT9 || ' E ON C.BUCKET_GROUP = E.BUCKET_GROUP AND C.BUCKET_ID = E.MAX_BUCKET_ID
    ...
    SELECT *, 
    CASE WHEN SUM(PD_SCALAR) OVER (PARTITION BY DOWNLOAD_DATE,EIL_MODEL_ID, PD_RULE_ID,BUCKET_ID,SEGMENTATION_ID ORDER BY FL_SEQ)  > 1 THEN CAST( 1 AS DOUBLE PRECISION) 
    ELSE CAST(SUM(PD_SCALAR) OVER (PARTITION BY DOWNLOAD_DATE,EIL_MODEL_ID, PD_RULE_ID,BUCKET_ID,SEGMENTATION_ID ORDER BY FL_SEQ) AS DOUBLE PRECISION) END  AS CUMULATIVE_PD_SCALAR2
    ...
    SELECT *,
    CAST(COALESCE(CUMULATIVE_PD_SCALAR2,0) AS DOUBLE PRECISION) - CAST(COALESCE(LAG(CUMULATIVE_PD_SCALAR2) OVER (PARTITION BY DOWNLOAD_DATE,EIL_MODEL_ID, PD_RULE_ID,BUCKET_ID,SEGMENTATION_ID ORDER BY FL_SEQ),0) AS DOUBLE PRECISION) AS PD_FINAL2
    ...
    AND A.EIL_MODEL_ID = B.EIL_MODEL_ID AND A.SEGMENTATION_ID = B.SEGMENTATION_ID AND A.PD_RULE_ID = B.PD_RULE_ID 
