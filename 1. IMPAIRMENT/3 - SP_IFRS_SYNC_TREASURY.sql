...
BEGIN 
    ...
    IF P_PRC = 'S' THEN
        ...
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'CREATE TABLE ' || V_TABLEINSERT1 || ' AS SELECT * FROM IFRS_MASTER_TREASURY WHERE 1=0 ';
        EXECUTE (V_STR_QUERY);
        ...
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'CREATE TABLE ' || V_TABLEINSERT2 || ' AS SELECT * FROM IFRS_JUDGEMENT_RATING_TREASURY WHERE 1=0 ';
        EXECUTE (V_STR_QUERY);
        ...
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'CREATE TABLE ' || V_TABLEINSERT3 || ' AS SELECT * FROM IFRS_MASTER_EXTERNAL_RATING WHERE 1=0 ';
        EXECUTE (V_STR_QUERY);
    END IF;
    ...
    ,MARGIN_RATE::DOUBLE PRECISION AS MARGIN_RATE      
    ,MARGIN_RECEIVABLE_CCY::NUMERIC AS MARGIN_ACCRUED      
    ...
    WHEN ''ANUALLY'' THEN ''Y''      
    END AS PAYMENT_TERM       
    ,CASE MARGIN_CALCULATION_CODE      
    WHEN ''ACTUAL/360'' THEN 8      
    WHEN ''ACTUAL/ACTUAL'' THEN 2      
    END AS MARGIN_CALCULATION_CODE       
