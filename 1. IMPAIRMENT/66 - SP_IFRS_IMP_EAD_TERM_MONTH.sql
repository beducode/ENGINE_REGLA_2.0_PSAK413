...
BEGIN 
    ...
    IF P_PRC = 'S' THEN
        ...
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'SELECT DUPLICATE_TABLE(LOWER(''IFRS_EAD_TERM_MONTHLY''),LOWER(''' || V_TABLEINSERT3 || '''), FALSE)';
        EXECUTE (V_STR_QUERY);
    END IF;
    ...
    A.UNAMORT_COST_AMT,              
    A.UNAMORT_FEE_AMT,              
    CASE WHEN N.N = 1 THEN A.MARGIN_ACCRUED ELSE 0 END AS MARGIN_ACCRUED,              
    A.UNUSED_AMOUNT,              
    A.FAIR_VALUE_AMOUNT,              
    A.EAD_BALANCE AS EAD_BALANCE,          
    A.PLAFOND,              
    CASE WHEN N.N <> 1 AND E.EAD_BALANCE LIKE  ''%MARGIN_ACCRUED%'' THEN               
    CASE WHEN COALESCE(A.EAD_BALANCE,0) - COALESCE(A.MARGIN_ACCRUED,0) < 0 THEN 0 ELSE COALESCE(A.EAD_BALANCE,0) - COALESCE(A.MARGIN_ACCRUED,0) END               
    ELSE               
