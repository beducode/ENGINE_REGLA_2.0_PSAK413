...
BEGIN 
    ...
    INNER JOIN IFRS_ECL_MODEL_DETAIL_PD B ON A.PKID = B.EIL_MODEL_ID 
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE INDEX IDX_CUMMULATIVE_PD_' || P_RUNID || ' ON CUMMULATIVE_PD_' || P_RUNID || ' (DOWNLOAD_DATE, EIL_MODEL_ID, SEGMENTATION_ID, PD_RULE_ID, BUCKET_ID, FL_SEQ) ';
    EXECUTE (V_STR_QUERY);
    ...
    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'CREATE INDEX IDX_PD_FINAL_' || P_RUNID || ' ON PD_FINAL_' || P_RUNID || ' (EIL_MODEL_ID, SEGMENTATION_ID, PD_RULE_ID, BUCKET_ID, FL_SEQ) ';
    EXECUTE (V_STR_QUERY);

    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'WITH CTE AS (
    SELECT B.PD_FINAL2, B.CUMULATIVE_PD_SCALAR2 , B.EIL_MODEL_ID, B.SEGMENTATION_ID, B.PD_RULE_ID, B.BUCKET_ID, B.FL_SEQ
    FROM ' || V_TABLEINSERT4 || ' A 
    INNER JOIN PD_FINAL_' || P_RUNID || ' B ON A.EIL_MODEL_ID = B.EIL_MODEL_ID AND A.SEGMENTATION_ID = B.SEGMENTATION_ID AND A.PD_RULE_ID = B.PD_RULE_ID 
    ...
    FROM CTE AS B
    WHERE A.EIL_MODEL_ID = B.EIL_MODEL_ID
