---- DROP PROCEDURE SP_IFRS_GL_OUTBOUND_SUMMARIZE;

CREATE OR REPLACE PROCEDURE SP_IFRS_GL_OUTBOUND_SUMMARIZE(
...
BEGIN 
    ...
    V_STR_QUERY := V_STR_QUERY || 'DELETE FROM ' || V_TABLEINSERT2 || ' 
        ...
        AND TRANSACTION_TYPE = ''AMORTIZATION'' ';
    EXECUTE (V_STR_QUERY);

    V_STR_QUERY := '';
    V_STR_QUERY := V_STR_QUERY || 'DELETE FROM ' || V_TABLEINSERT3 || ' 
        WHERE TRANSACTION_TYPE = ''AMORTIZATION'' ';
    EXECUTE (V_STR_QUERY);
    ...
    IF V_EXISTS THEN
        ...
        V_STR_QUERY := V_STR_QUERY || 'INSERT INTO ' || V_TABLEINSERT2 || ' 
            (                
                ...
                ,PRODUCT_CODE                
                ,CUSTOMER_TYPE                
                ,SUM(AMOUNT_LEV) / SUM(AMOUNT) AS TRANSACTION_RATE                
    ...
    V_STR_QUERY := V_STR_QUERY || 'INSERT INTO ' || V_TABLEINSERT3 || ' 
        (                  
            ...
            ,SOURCE_DATA                
            ,EVENT_TYPE            
            ,''AMORTIZATION'' AS TRANSACTION_TYPE                  
        FROM ' || V_TABLEINSERT1 || ' 
        WHERE EXTRACT(YEAR FROM BUSS_DATE) = EXTRACT(YEAR FROM ''' || CAST(V_CURRDATE AS VARCHAR(10)) || '''::DATE) 
            AND EXTRACT(MONTH FROM BUSS_DATE) = EXTRACT(MONTH FROM ''' || CAST(V_CURRDATE AS VARCHAR(10)) || '''::DATE) 
            AND CLASS IN (''A'', ''L'') 
    ...
    RAISE NOTICE 'SP_IFRS_GL_OUTBOUND_SUMMARIZE | AFFECTED RECORD : %', V_RETURNROWS2;
    ---------- ====== BODY ======

    -------- ====== LOG ======
    V_TABLEDEST = V_TABLEINSERT3;
    V_COLUMNDEST = '-';
    V_SPNAME = 'SP_IFRS_GL_OUTBOUND_SUMMARIZE';
    V_OPERATION = 'INSERT';
