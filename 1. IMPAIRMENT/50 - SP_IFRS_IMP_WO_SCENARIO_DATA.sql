---- DROP PROCEDURE SP_IFRS_IMP_WO_SCENARIO_DATA;

CREATE OR REPLACE PROCEDURE SP_IFRS_IMP_WO_SCENARIO_DATA(
...
BEGIN 
    ...
    IF P_PRC = 'S' THEN 
        ...
        V_TABLEINSERT4 := 'IFRS_WO_SCENARIO_DATA_' || P_RUNID || '';
    ...
    ELSE 
        ...
        V_TABLEINSERT4 := 'IFRS_WO_SCENARIO_DATA';
    ...
    IF P_PRC = 'S' THEN
        ...
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'CREATE TABLE ' || V_TABLEINSERT4 || ' AS SELECT * FROM IFRS_WO_SCENARIO_DATA WHERE 0=1';
        EXECUTE (V_STR_QUERY);
    END IF;
    -------- ====== PRE SIMULATION TABLE ======
    ...
    LOOP
        V_STR_QUERY := '';
        V_STR_QUERY := V_STR_QUERY || 'INSERT INTO WO_' || P_RUNID || '         
        (              
        DOWNLOAD_DATE         
        ,LGD_RULE_ID         
        ,LGD_RULE_NAME         
    ...
    RAISE NOTICE 'SP_IFRS_IMP_WO_SCENARIO_DATA | AFFECTED RECORD : %', V_RETURNROWS2;
    -------- ====== BODY ======

    -------- ====== LOG ======
    V_TABLEDEST = V_TABLEINSERT4;
    V_COLUMNDEST = '-';
    V_SPNAME = 'SP_IFRS_IMP_WO_SCENARIO_DATA';
    V_OPERATION = 'INSERT';
